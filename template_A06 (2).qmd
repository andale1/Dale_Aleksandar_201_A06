...
[https://github.com/andale1/Dale_Aleksandar_201_A06](https://github.com/andale1/Dale_Aleksandar_201_A06)
...
```{r}
lebron_points_indexing <- player_info[[2]]$points
print(lebron_points_indexing)
lebron_points_names <- player_info[["LJ"]]$points
print(lebron_points_names)
```
...
```{r}
results <- sapply(points_per_game, round, 2)
results
```
...
mat_rand_1 <- apply(mat_rand, MARGIN = 1:2, rounder)
mat_rand_1
```
...
```{r}
month_avg_temps <- apply(max_temps[,-c(1:2)], 2, mean)
print(month_avg_temps)
```
...
```{r}
ohio_data <- max_temps[max_temps$state == "OH", ]
colorado_data <- max_temps[max_temps$state == "CO",]
virginia_data <- max_temps[max_temps$state == "VA",]
...
```{r}
annual_medians <- apply(colorado_data[,-c(1:2)], 1, median)
top_1 <- which.max(annual_medians)
bottom_1 <- which.min(annual_medians)

print(paste0("The maximum median temperature of ", max(annual_medians),
" occured in ", colorado_data$year[top_1], " and the minimum median annual temperature of ", min(annual_medians), " occured in ",
colorado_data$year[bottom_1], "."))
...
```{r}
digit_sum1 <- function(x){
if(x < 10){
return(x)
}else{
return(x %% 10 + digit_sum1(x %/% 10))
}
}

result <- digit_sum1(935068)
print(result)
```
